	...
	<div class="content-box-large">
		...
		<div class="panel-body">
			...
			<fieldset>
				...
				<div class="form-group col-sm-3">
					<label>Date</label>
					<div class="bfh-datepicker" data-name="sale_date" data-format="d-m-y" data-date="{{ date('Y-m-d', strtotime($master_data->sale_date)) }}">
				...
				<div class="form-group col-sm-2">
					<label>Bilty #</label>
					<input class="form-control" placeholder="Bilty #" maxlength="15" name="bilty_no" type="text" value="{{ $master_data->bilty_no }}">
				</div>
				<div class="form-group col-sm-4">
					<label>Cargo Address</label>
					<input class="form-control" placeholder="Cargo Address" maxlength="15" name="adda_address" type="text" value="{{ $master_data->adda_address }}">
				</div>
				<div class="panel-heading">
					<legend>Edit All Sale Items<span style="float:right;" id="current_stock"></span></legend>
				</div>
			...
			<div class="table-responsive">
              ...
              <tbody id="addNewItems">
              	<?php
              		...
              		{
              		  $str  = "<tr id='Product_".$detail->product_id."'>";
	                   $str .= "<td style=text-align:left;'>".$detail->product_name."<input type='hidden' name='product_id[]' value='".$detail->product_id."'></td>";
	                   $str .= "<td>".number_format($detail->quantity)."<input id='Quantity_".$detail->product_id."' type='hidden' name='quantity[]' value='".$detail->quantity."'></td>";
	                   $str .= "<td>".number_format($detail->list_price,2)."<input type='hidden' name='list_price[]' value='".$detail->list_price."'></td>";
	                   $str .= "<td>".$detail->discount."%<input type='hidden' name='discount_id[]' value='".$detail->discount."'></td>";
	                   $str .= "<td>".number_format($detail->net_price,2)."<input id='NetPrice_".$detail->product_id."' type='hidden' name='net_price[]' value='".$detail->net_price."'></td>";
	                   $str .= "<td>".number_format($detail->total_amount,2)."<input id='TotalAmount_".$detail->product_id."' type='hidden' name='total_amount[]' value='".$detail->total_amount."'></td>";
	                   $str .= "<td class='text-center'><img class='cursor deleteItems' src='/images/delete.png' alt='Delete' id='".$detail->product_id."'></td>";
	                  $str .= "</tr>";
	                  echo $str;
	              	}
			...
			<div class="table-responsive" id="div_total_detail">
				<table class="table">
              <thead>
                <tr>
                  <th style="width:193px;">Total Items:&nbsp;<span id="text_total_item">{{ number_format($master_data->total_item) }}</span></th>
                  <th>Total Quantity:&nbsp;<span id="text_total_quantity">{{ number_format($master_data->total_quantity) }}</span></th>
                  <th></th>
                  <th></th>
                  <th></th>
                  <th>Grand Total Amount:&nbsp;<span id="text_total_amount">{{ number_format($master_data->grand_total,2) }}</span></th>
                  <th></th>
                  <th></th>
                </tr>
              </thead>
            </table>
			<input type="hidden" name="total_quantity" id="grand_total_quantity" value="{{ number_format($master_data->total_quantity) }}">
            <input type="hidden" name="grand_total" id="grand_total_amount" value="{{ number_format($master_data->grand_total,2) }}">
            <input type="hidden" name="total_item" id="grand_total_item" value="{{ number_format($master_data->total_item) }}">
			</div>
			<div class="form-group col-sm-3">
...
<input type="hidden" id="item_counter" name="item_counter" value="{{ $master_data->total_item }}" />
<style type="text/css">
.amount_text {
	text-align: center;
    font-size: 14px;
...
@section('custom_js')
	<script type="text/javascript">
		...
		$('#product_id .get_selected_options a').on('click', dropdownToggle);
		});
