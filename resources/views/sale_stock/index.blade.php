		...
		<div class="panel-body">
			...
			<thead>
				<tr>
					...
					<th>Party Name</th>
					<th>Bilty #</th>
					<th class="text-center">Total Items Qty</th>
			...
			<tbody>
				@foreach($arrayStock as $stock)
					<tr class="odd gradeX" id="row_{{ $stock->id }}">
						...
						<td class="text-center">{{ number_format($stock->grand_total,2) }}</td>
						<td>{{ $stock->user_name }}</td>
						<td class="center"><a href="/sale_stock/{{ $stock->id }}/edit"><img src="/images/edit.png" alt="Edit"></a>&nbsp; &nbsp;<a id="{{ $stock->id }}" class="deleteRecord"><img src="/images/delete.png" alt="Delete" style="cursor:pointer;"></a></td>
					</tr>
				@endforeach
			</tbody>
			</table>
				...
				$("#dialog-confirm-delete").dialog({
								...
								buttons: {
									Delete: function() {
										...
										$.ajax({
                          type: "GET",
                      				url: '/sale_stock/delete_sale_stock',
                          data: { DelID: DelID }
                      }).done(function( msg ) {
                          //alert( msg+'ttttt' );
                          if(msg == "delete")
