...
class Report extends Model
{
    ...
    {
    	$arrayDetail = DB::table('products_stock')
		->join('products', 'products.id', '=', 'products_stock.product_id')
		->select(DB::raw('(SUM(product_credit) - SUM(product_debit)) AS TotalStock, products.name AS product_name')) // , products.name AS product_name, products.id AS product_id
		->groupBy('product_id')
